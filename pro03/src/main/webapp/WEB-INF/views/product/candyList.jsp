		...
		<section class="page clr-fix" id="page1">
			<div class="page-wrap">
				<div style="margin:10px 0; margin-left:100px;">
					...
					<a href="${path2 }/product/categoryList.do?category=candy"><button class="add-product-btn-checked">캔디</button></a>
					<h2>캔디 리스트</h2>
				</div>
			...
			<div class="paginations-container">
				<div class="paginations">
			        <a href="?category=candy&page=1" class="page-link">&lt;&lt;</a>
			        ...
			        <c:forEach begin="${pagingBean.getStartPageOfPageGroup()}" end="${pagingBean.getEndPageOfPageGroup()}" var="i">
			            <c:choose>
			                <c:when test="${i == pagingBean.getNowPage()}">
			                    <a href="?category=candy&page=${i}" class="page-link active">${i}</a>
			                </c:when>
			                <c:otherwise>
			                    <a href="?category=candy&page=${i}" class="page-link">${i}</a>
			                </c:otherwise>
			            </c:choose>
			        </c:forEach>
			...
			<script>
		    	...
		    	function goToNextPageGroup() {
		    		...
		    		if(nextPageGroupStart <= totalPage) {
		    			window.location.href = "?category=candy&page=" + nextPageGroupStart; //다음 페이지 그룹으로 이동
		    		}
		    	}
		    	...
		    	function goToPreviousPageGroup() {
		    		...
		    		if(previousPageGroupStart >= 1) {
		    			window.location.href = "?category=candy&page=" + previousPageGroupStart;
		    		}
		    	}
		    </script>
		</section>	
